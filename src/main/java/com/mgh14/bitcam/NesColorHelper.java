...
//    };
    ...
    static int findClosestColor(final int pixel, final int[][] colors) {
        ...
        for (int i = 0; i < colors.length; i++)
        {
            ...
            int totalDistance = Math.abs(red - colorRed) + Math.abs(green - colorGreen) +
                    Math.abs(blue - colorBlue);
...
//                blue + ") is " + COLORS[closestIndex]);
    ...
    static int rgbToInt(final int red, final int green, final int blue)
    {
        int pixel = 0;
        pixel += (red << 16);
        pixel += (green << 8);
